...
async function processRecurringBooking(job: Job<RecurringBookingJobData>) {
  ...
  log.info({ recurringBookingId }, "Processing recurring booking")
  ...
  if (!recurring.isActive) {
    log.info({ recurringBookingId }, "Recurring booking is not active")
  ...
  if (nextInstanceNumber > totalInstances) {
    log.info(
      { recurringBookingId },
      "All instances created for recurring booking"
    )
    await db
  ...
  if (recurring.endDate && nextDate > recurring.endDate) {
    log.info({ recurringBookingId }, "End date reached for recurring booking")
    await db
      .update(recurringBookings)
      .set({ isActive: false, updatedAt: new Date() })
      .where(eq(recurringBookings.id, recurringBookingId))
  ...
  log.info(
    ...
    "Created instance for recurring booking"
  )
...
recurringBookingWorker.on("completed", (job) => {
  log.info({ jobId: job.id }, "Recurring booking job completed")
})

recurringBookingWorker.on("failed", (job, err) => {
  log.error({ jobId: job?.id, err }, "Recurring booking job failed")
})
