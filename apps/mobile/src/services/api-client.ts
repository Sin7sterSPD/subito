...
function defaultTimeoutFor(method: string, path: string): number {
...
function shouldRetry(attempt: number, err: unknown, status?: number): boolean {
...
class ApiClient {
    ...
    while (attempt < 4) {
      ...
      try {
        if (
          !skipRefresh &&
          this.authToken &&
          isAccessTokenExpiringSoon(this.authToken)
        ) {
          await singleFlightRefresh()
        }
        const response = await this.fetchOnce(url, init, timeoutMs)
        const payload = (await parseResponseBody(response)) as Record<
          string,
          unknown
        > | null
  ...
  async upload<T>(
    ...
    return this.requestInner<T>(endpoint, "POST", formData, {
      isMultipart: true,
    })
  }
}
