...
interface CreateBookingData {
  ...
  /** When set, API atomically reserves capacity on that slot (from GET /bookings/slots). */
...
export const bookingsApi = {
  ...
  getSlots: (params: {
    ...
    if (params.bookingType)
      searchParams.append("bookingType", params.bookingType)
  ...
  getPartnerLocation: (bookingId: string) =>
    apiClient.get<{ location: PartnerLocation | null }>(
      `/bookings/${bookingId}/partner-location`
    ),

  getChildBookings: (bookingId: string) =>
    apiClient.get<{ parentBooking: Booking; instances: unknown[] }>(
      `/bookings/child-bookings/${bookingId}`
    ),

  getRefundDetails: (bookingId: string) =>
    apiClient.get<{ booking: Booking; refunds: RefundDetails[] }>(
      `/bookings/refund/${bookingId}`
    ),

  getBridgingContext: (bookingId: string) =>
    apiClient.get<{ bookingId: string; status: string; partnerId?: string }>(
      `/bookings/bridging/context/instance/${bookingId}`
    ),
  ...
  createBooking: (data: CreateBookingData, idempotencyKey?: string) =>
    apiClient.post<{
      ...
      order: { id: string; orderNumber: string; amount: string } | null
    }>("/bookings", data, idempotencyKey ? { idempotencyKey } : {}),

  cancelBooking: (data: CancelBookingData) =>
    apiClient.post<{ cancelled: boolean; bookingId: string }>(
      "/bookings/cancel",
      data
    ),
  ...
  extendBooking: (
    ...
    apiClient.post<{
      bookingId: string
      extensionOrder: { id: string; orderNumber: string; amount: string }
      newTotal: string
    }>("/bookings/extend", data, idempotencyKey ? { idempotencyKey } : {}),

  rescheduleInstance: (instanceId: string, data: RescheduleData) =>
    apiClient.post<{ rescheduled: boolean; instanceId: string }>(
      `/bookings/instance/${instanceId}/reschedule`,
      data
    ),

  cancelInstance: (instanceId: string, data: { reason: string }) =>
    apiClient.put<{ cancelled: boolean; instanceId: string }>(
      `/bookings/cancelInstance/${instanceId}`,
      data
    ),
}
