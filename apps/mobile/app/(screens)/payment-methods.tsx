...
import { PaymentMethodsList, type PaymentSection } from '../../src/components/payment/payment-method-list';
...
export default function PaymentMethodsScreen() {
  const { orderId: oid, bookingId: bid, amount: amt } = useLocalSearchParams<{
    orderId: string;
    bookingId: string;
    amount: string;
  }>();
  ...
  const setIsPaymentProviderOpen = usePaymentsStore((s) => s.setIsPaymentProviderOpen);
  ...
  for (const app of installedUpi) {
    upiRows.push({
      ...
      leading: <Ionicons name="phone-portrait-outline" size={22} color={semantic.primary} />,
      onPress: () => {
  ...
  const cardRows: PaymentSection['rows'] =
    paymentOptions?.cards?.length ?
      paymentOptions.cards.map((c) => ({
        key: c.id,
        title: c.name,
        subtitle: 'Credit or debit card',
        leading: <Ionicons name="card-outline" size={22} color={semantic.primary} />,
        onPress: () => {
          sendEventAnalytics(PAYMENT_EVENTS.PAYMENT_INSTRUMENT_SELECTED, {
            Type: 'Card',
            Subtype: c.name,
  ...
  const nbRows: PaymentSection['rows'] =
    paymentOptions?.netbanking.map((c) => ({
      ...
      leading: <Ionicons name="business-outline" size={22} color={semantic.primary} />,
      onPress: () => {
        sendEventAnalytics(PAYMENT_EVENTS.PAYMENT_INSTRUMENT_SELECTED, {
          Type: 'Other',
          Subtype: 'NETBANKING',
        });
        Alert.alert('Coming soon', 'Net banking will be available in a future update.');
      },
    })) ?? [];
  ...
  const walletRows: PaymentSection['rows'] =
    paymentOptions?.wallets.map((c) => ({
      ...
      title: c.name,
      subtitle: 'Wallet',
      leading: <Ionicons name="wallet-outline" size={22} color={semantic.primary} />,
      onPress: () => {
        sendEventAnalytics(PAYMENT_EVENTS.PAYMENT_INSTRUMENT_SELECTED, {
          Type: 'Other',
          Subtype: `WALLET:${c.name}`,
        });
        Alert.alert('Coming soon', 'Wallet payments will be available in a future update.');
      },
    })) ?? [];
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.center}>
          ...
          <Button variant="primary" onPress={() => router.back()} style={styles.mt}>
            Go back
          </Button>
        </View>
      </SafeAreaView>
