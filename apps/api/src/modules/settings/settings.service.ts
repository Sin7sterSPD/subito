...
import { appSettings, featureFlags, complaints } from "@subito/db"
...
export async function getAppSettings() {
  ...
  const result: AppSettingsResponse = {
    ...
    isInstantBookingDisabled: !(flagsMap["instant_booking"] ?? true),
    isScheduleBookingDisabled: !(flagsMap["scheduled_booking"] ?? true),
    isRecurringBookingDisabled: !(flagsMap["recurring_booking"] ?? true),
...
export async function submitComplaint(
  ...
  if (!complaint) {
    throw new InternalError("Failed to submit complaint")
  }
