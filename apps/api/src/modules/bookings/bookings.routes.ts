...
bookingsRouter.get(
  "/get-user-latest-booking/:userId",
  requireAuth,
  async (c) => {
...
bookingsRouter.get("/bridging/context/instance/:id", requireAuth, async (c) => {
  ...
  const bookingId = c.req.param("id")
  const context = await bookingsService.getBridgingContext(
    bookingId,
    userId,
    role
  )
...
bookingsRouter.post(
  ...
  async (c) => {
    ...
    const idempotencyKey = c.req.header("Idempotency-Key")
    const data = c.req.valid("json")
    const result = await bookingsService.extendBooking(
      userId,
      data,
      idempotencyKey
    )
...
bookingsRouter.put(
  ...
  async (c) => {
    ...
    const instanceId = c.req.param("id")
    const data = c.req.valid("json")
    const result = await bookingsService.cancelInstance(
      userId,
      instanceId,
      data
    )
