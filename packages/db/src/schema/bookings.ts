...
import { catalogs, bundles } from "./listings.js"
import { coupons } from "./coupons.js"
import { hubs, microHubs } from "./hubs.js"
import { bookingTypeEnum, recurringTypeEnum } from "./carts.js"
...
export const bookings = pgTable(
  "bookings",
  {
    ...
    surgeAmount: decimal("surge_amount", { precision: 12, scale: 2 }).default(
      "0"
    ),
  ...
  (table) => ({
    userIdStatusIdx: index("bookings_user_id_status_idx").on(
      table.userId,
      table.status
    ),
    partnerIdIdx: index("bookings_partner_id_idx").on(table.partnerId),
    partnerScheduleUnique: uniqueIndex("bookings_partner_schedule_unique")
      .on(table.partnerId, table.scheduledDate, table.scheduledStartTime)
      .where(sql`${table.status} NOT IN ('CANCELLED', 'REFUNDED')`),
  })
)
...
export const idempotencyKeys = pgTable(
  ...
  (table) => ({
    uniqueKeyUserResource: unique().on(
      table.key,
      table.userId,
      table.resourceType
    ),
