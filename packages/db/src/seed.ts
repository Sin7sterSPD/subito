...
async function seed() {
  ...
  const [customer1, customer2, customer3, customer4] = allCustomers
  ...
  const microHubs = await db
    ...
    .returning()
  const [banjaraHillsMH, madhapurMH] = microHubs
  ...
  const [bathroomCleaning] = await db
    .insert(schema.categories)
    .values({
  ...
  const [kitchenCleaning] = await db
    .insert(schema.categories)
    .values({
  ...
  const [bathroomListing] = await db
    .insert(schema.listings)
    .values({
  ...
  const [catalogDeep2BHK] = await db
    .insert(schema.catalogs)
    .values({
  ...
  const [catalogBathroom] = await db
    .insert(schema.catalogs)
    .values({
  ...
  // Recurring booking
  const [recurringBooking] = await db
    .insert(schema.bookings)
    .values({
  ...
  const [recurringConfig] = await db
